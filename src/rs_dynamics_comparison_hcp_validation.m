...
%                 (i-1)*n_dim+1:i*n_dim)./lambda1_iidx(:, i)';
            psi1_temp = K*embed_iidx(:, (i-1)*n_dim+1:i*n_dim)./lambda1_iidx(:, i)';
            psi1_temp = reshape(psi1_temp, n_tp_rest, n_dim);
            psi1(:, :, i) = psi1_temp;
        end
        toc
        psi1 = reshape(psi1, n_tp_rest, n_dim*n_sub);
        % second round embedding
        embed_temp = zeros(n_dim*n_sub, n_tp_all);
        embed_temp(:, train_range) = reshape(embed_all(:,...
            all_sex==i_idx, :), [], n_sub*n_dim)';
        embed_temp(:, test_range) = psi1';
        configAffParams3_temp = configAffParams3;
        configAffParams3_temp.sig = sigma2_all;
        [K, ~] = calcAffinityMat(embed_temp, configAffParams3_temp);
        K = K(test_range, train_range);
        K = K./sum(K, 2);
        psi2 = K*dm_all'./lambda2_all';

        dm_temp_all = zeros(n_dim, n_tp_all);
        dm_temp_all(:, train_range) = dm_all;
